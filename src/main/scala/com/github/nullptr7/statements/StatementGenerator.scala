...
// Creating a class with Quotes argument allows sharing quotes and reflect import across all the
// methods of the class and easily split the logic.
class StatementGenerator(using Quotes) {
  // In case of problems with finding the file, set it to an absolute path.
  private val SchemaPath = "sql/schema.sql"
  ...
  private def validateWithSchema(
    ...
    val schemaColumns = parsed.getOrElse(
      tableName,
      report.throwError(
        "Schema doesn't have table defined with name: " + tableName
      ),
    )
  ...
  private def parseColumDef(columnDefTerm: Term): ColumnInfo =
    ...
    // Parameter 'a' captures the type and $name expression representing the name argument of the constructor.
    columnDefTerm.asExprOf[ColDef[_]] match {

      case '{ ColDef[a]($name) } =>
        val paramType = TypeRepr.of[a]

        // It is possible (and for this case required) to get compile time values. It
        // will fail in case of the name is a variable.
        val nameValue = name.valueOrError
        println(
          s"Type: ${Type.show[a]} , value: $nameValue, name AST: ${name
              .asTerm
  ...
  private def parseColumnInfo(paramMapping: Term): Seq[ColumnInfo] =
    ...
    paramMapping match {
      ...
      case _ =>
        report.throwError(
          "Provide ColDef list e.g.: StatementGenerator.createPreparedStatement(ColDef[Int](\"c1\"), ColDef[String](\"c2\"))"
        )
    }
  ...
  def createPreparedStatementImpl[A <: Tuple: Type](
    ...
    // There are using for output, but println and throwing custom errors also work.
    println(
      "Generating prepared statement for arguments: " + columnMapping
        .asTerm
        .show(using Printer.TreeStructure)
    )
    ...
    // As the types were verified, an unsafe statement can be safely created and passed to the
...
object StatementGenerator {
  ...
  // Auxiliary function to create the class as the inlined main maro function needs a single expression.
  ...
  inline def createPreparedStatement[A <: Tuple](
